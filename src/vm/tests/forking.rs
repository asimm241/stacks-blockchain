...
fn test_at_block_mutations() {
    ...
    fn initialize(owned_env: &mut OwnedEnvironment) {
        ...
        owned_env
            .initialize_contract(c.clone(), &contract, &OptionalData { data: None })
            .unwrap();
    }
    ...
    fn branch(
        ...
        owned_env
            .execute_transaction(p1, Value::none(), c, &vec![], to_exec)
            .map(|(x, _, _)| x)
    }
...
fn test_at_block_missing_defines() {
    fn initialize_1(owned_env: &mut OwnedEnvironment) {
        ...
        eprintln!("Initializing contract...");
        owned_env
            .initialize_contract(c_a.clone(), &contract, &OptionalData { data: None })
            .unwrap();
    }
    ...
    fn initialize_2(owned_env: &mut OwnedEnvironment) -> Error {
        ...
        eprintln!("Initializing contract...");
        let e = owned_env
            .initialize_contract(c_b.clone(), &contract, &OptionalData { data: None })
            .unwrap_err();
        e
    }
...
fn initialize_contract(owned_env: &mut OwnedEnvironment) {
    ...
    owned_env
        .initialize_contract(contract_identifier, &contract, &OptionalData { data: None })
        .unwrap();
}
